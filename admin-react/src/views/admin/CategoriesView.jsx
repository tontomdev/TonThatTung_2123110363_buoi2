...
export default function CategoriesView() {
    ...
    const deleteCategory = async (id) => {
        if (window.confirm('Ban co chac muon xoa muc nay?')) {
                ...
                <table className="data-table">
                    ...
                    <tbody>
                        {loading ? (
                            ...
                            <tr><td colSpan="4" className="text-center text-muted">Khong co danh muc nao.</td></tr>
                        ) : (
                            categories.map(cat => (
                                <tr key={cat.id}>
                                    <td>#{cat.id}</td>
                ...
                <div className="modal-overlay active">
                    <div className="modal-content glass-panel">
                        <div className="modal-header">
                            <h2>{formData.id ? 'Sua danh muc' : 'Them danh muc'}</h2>
                        ...
                        <div className="modal-body">
                            <form onSubmit={handleSubmit}>
                                <div className="form-group">
                                    ...
                                    <input type="text" required className="form-control" placeholder="VD: Do uong da xay"
                                        value={formData.categoryName} onChange={e => setFormData({ ...formData, categoryName: e.target.value })} />
                                </div>
                                <div className="form-group">
                                    <label>Mo ta</label>
                                    <textarea className="form-control" placeholder="Mo ta ngan..."
                                        value={formData.description} onChange={e => setFormData({ ...formData, description: e.target.value })} />
                                </div>
                                <div className="modal-actions">
                                    <button type="button" className="btn btn-secondary" onClick={() => setShowModal(false)}>Huy</button>
                                    <button type="submit" className="btn btn-primary">Luu danh muc</button>
                                </div>
                            </form>
                        </div>
                    </div>
